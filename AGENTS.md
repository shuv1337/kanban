...
- Prefer SDK-provided types, schemas, helpers, and model metadata over local redefinitions. For external agent integrations, use the upstream source of truth whenever possible instead of recreating unions, support checks, or shapes in Shuvban.
...
web-ui Stack
- Shuvban web-ui uses Tailwind CSS v4 for styling, Radix UI for accessible headless primitives, and Lucide React for icons.
...
- Shuvban now uses the PTY-backed CLI path for supported agents. When agent behavior is unclear, inspect `src/terminal/`, `src/prompts/append-system-prompt.ts`, and the relevant launch/test files instead of assuming there is a separate native runtime path.
...
- If CI hangs on Node 22 after tests seem to finish, suspect a live subprocess, lingering PTY session, or hook-launched child process before assuming a slow test body. Read `.plan/docs/node22-ci-hanging-tests-investigation.md` before repeating that investigation.
- When Shuvban runs on a headless remote Linux instance (for example over SSH+tunnel), native folder picker commands may be unavailable (`zenity`/`kdialog`). Treat this as a normal remote-runtime limitation and use manual path entry fallback instead of requiring desktop packages.
- pi is first-class and stays on the PTY-backed CLI path. The non-obvious parts are: Shuvban generates a pi extension at `~/.shuvban/hooks/pi/shuvban-extension.ts`, task sessions use deterministic encoded `--session-dir` paths (see `src/terminal/pi-session-paths.ts`) so trash-restore can safely use `--continue`, and home sidebar sessions must use `--no-session` instead of `--session-dir` because home agent ids are synthetic/ephemeral and would otherwise leak one-off persisted histories.
