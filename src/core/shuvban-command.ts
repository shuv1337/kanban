...
function looksLikeEntrypointPath(value: string): boolean {
	...
	return /shuvban(?:\.(?:cmd|ps1|exe))?$/iu.test(value);
}

export function resolveShuvbanCommandParts(
...
export function buildShuvbanCommandParts(
	args: string[],
	context: RuntimeInvocationContext = {
		execPath: process.execPath,
		argv: process.argv,
	...
	return [...resolveShuvbanCommandParts(context), ...args];
}
