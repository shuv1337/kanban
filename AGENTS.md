...
- pi is first-class but stays on the PTY-backed CLI path in v1, not the native Cline path. The non-obvious parts are: Kanban generates a pi extension at `~/.cline/kanban/hooks/pi/kanban-extension.ts`, task sessions use deterministic encoded `--session-dir` paths (see `src/terminal/pi-session-paths.ts`) so trash-restore can safely use `--continue`, and home sidebar sessions must use `--no-session` instead of `--session-dir` because home agent ids are synthetic/ephemeral and would otherwise leak one-off persisted pi histories.
